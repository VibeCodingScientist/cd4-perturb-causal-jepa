...
![Marson CD4 Perturb-seq predictability scorecard — seven probes calibrated to the reliability ceiling; six sit at the floor, C2 is the one positive](figures/predictability_scorecard.svg)
...
| result | value | committed in | generated by |
|---|---|---|---|
| Do-operator control (C2, within-dataset) | **+0.118** cond / **+0.162** gene (Pearson-δ, top-50 DEG) | `results/benchmark_table.csv` | `scripts/run_cp1.py` |
| Predictability floor (cross-donor) | 0.032 cond / 0.049 gene (perm *p* < 0.001) | `results/budget_cross_donor.csv` | `scripts/run_cp1.py` |
| Fraction of the reliability ceiling (gene) | do-operator **0.56** · Ridge collapses to **0.02** | `results/fraction_of_ceiling.csv` | `scripts/make_scorecard_figure.py` |
| External causal-edge validation (G-F.2) | causal **0.559** = twin **0.569** → **−0.010** (cluster CI [−0.013, −0.005]) | `results/fusion_gf2.csv` | `scripts/fusion_gf2.py` + `verify_gf2.py` |
| Data-integrity C2 control (restored data) | **+0.106 / +0.156** vs committed +0.118/+0.162 | `results/c2_control.csv` | `scripts/c2_control.py` |
| Second-dataset port (Schmidt 2022) | R1 **0.71–0.75** · R2 **0.93** · R3 **0.86–0.93** (all *p* ≤ 1/501) | `results/gpa2_scorecard.csv` | `scripts/gpa2_stage2b_probes.py` |
